...
/**
 * {@code View} that displays a {@link SplashScreen} until a given {@link PinkFlutterView} renders its
 * first frame.
 */
/* package */ final class PinkFlutterSplashView extends FrameLayout {
    private static String TAG = "PinkFlutterSplashView";

    @Nullable
    private SplashScreen splashScreen;
    @Nullable
    private PinkFlutterView flutterView;
    @Nullable
    ...
    @NonNull
    private final PinkFlutterView.FlutterEngineAttachmentListener flutterEngineAttachmentListener =
            new PinkFlutterView.FlutterEngineAttachmentListener() {
                @Override
    ...
    public PinkFlutterSplashView(@NonNull Context context) {
        this(context, null, 0);
    }

    public PinkFlutterSplashView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PinkFlutterSplashView(
     ...
     *
     * <p>If no {@code splashScreen} is provided, this {@code PinkFlutterSplashView} displays the given
     * {@code flutterView} on its own.
     */
    public void displayFlutterViewWithSplash(
            @NonNull PinkFlutterView flutterView, @Nullable SplashScreen splashScreen) {
        // If we were displaying a previous PinkFlutterView, remove it.
        ...
        // Display the new PinkFlutterView.
        ...
        if (splashScreen != null) {
            if (isSplashScreenNeededNow()) {
                ...
                // This is the typical case. A FlutterEngine is attached to the PinkFlutterView and we're
                ...
                Log.v(
                        TAG,
                        "PinkFlutterView is not yet attached to a FlutterEngine. Showing nothing until a FlutterEngine is attached.");
        ...
        if (flutterView == null) {
            throw new IllegalStateException(
                    "Cannot determine if previous splash transition was "
                            + "interrupted when no PinkFlutterView is set.");
        }
        if (!flutterView.isAttachedToFlutterEngine()) {
            throw new IllegalStateException(
                    "Cannot determine if previous splash transition was "
                            + "interrupted when no FlutterEngine is attached to our PinkFlutterView. This question "
    ...
    private boolean hasSplashCompleted() {
        if (flutterView == null) {
            throw new IllegalStateException(
                    "Cannot determine if splash has completed when no PinkFlutterView " + "is set.");
        }
        if (!flutterView.isAttachedToFlutterEngine()) {
            throw new IllegalStateException(
                    "Cannot determine if splash has completed when no "
                            + "FlutterEngine is attached to our PinkFlutterView. This question depends on an isolate ID "
     ...
     *
     * <p>This method requires that our PinkFlutterView be attached to an engine, and that engine have a
