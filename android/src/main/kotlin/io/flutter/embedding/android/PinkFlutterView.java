 ...
 *
 * <p>A {@code PinkFlutterView}'s UI is painted by a corresponding {@link FlutterEngine}.
 *
 * <p>A {@code PinkFlutterView} can operate in 2 different {@link
 ...
 *       PinkFlutterView} in this mode cannot be positioned between 2 other Android {@code View}s in the
 ...
 *       io.flutter.embedding.android.RenderMode#surface}, but a {@code PinkFlutterView} in this mode
 ...
 */
public class PinkFlutterView extends FrameLayout implements MouseCursorPlugin.MouseCursorViewDelegate {
    private static final String TAG = "PinkFlutterView";
    ...
    /**
     * Constructs a {@code PinkFlutterView} programmatically, without any XML attributes.
     *
     * <p>
     *
     * <ul>
     ...
     * </ul>
     * <p>
     * {@code PinkFlutterView} requires an {@code Activity} instead of a generic {@code Context} to be
     * compatible with {@link PlatformViewsController}.
     */
    public PinkFlutterView(@NonNull Context context) {
        this(context, null, new FlutterSurfaceView(context));
    }

    /**
     * Deprecated - use {@link #PinkFlutterView(Context, FlutterSurfaceView)} or {@link
     * #PinkFlutterView(Context, FlutterTextureView)} or {@link #PinkFlutterView(Context, FlutterImageView)}
     * instead.
     */
    @Deprecated
    public PinkFlutterView(@NonNull Context context, @NonNull RenderMode renderMode) {
        super(context, null);
    ...
    /**
     * Deprecated - use {@link #PinkFlutterView(Context, FlutterSurfaceView)} or {@link
     * #PinkFlutterView(Context, FlutterTextureView)} instead, and configure the incoming {@code
     * FlutterSurfaceView} or {@code FlutterTextureView} for transparency as desired.
     *
     * <p>Constructs a {@code PinkFlutterView} programmatically, without any XML attributes, uses a {@link
     * FlutterSurfaceView} to render the Flutter UI, and allows selection of a {@code
     * transparencyMode}.
     *
     * <p>{@code PinkFlutterView} requires an {@code Activity} instead of a generic {@code Context} to be
     * compatible with {@link PlatformViewsController}.
     */
    @Deprecated
    public PinkFlutterView(@NonNull Context context, @NonNull TransparencyMode transparencyMode) {
        this(
                context,
                null,
    ...
    /**
     * Constructs a {@code PinkFlutterView} programmatically, without any XML attributes, uses the given
     ...
     * compatible with {@link PlatformViewsController}.
     */
    public PinkFlutterView(@NonNull Context context, @NonNull FlutterSurfaceView flutterSurfaceView) {
     ...
     * compatible with {@link PlatformViewsController}.
     */
    public PinkFlutterView(@NonNull Context context, @NonNull FlutterTextureView flutterTextureView) {
     ...
     */
    @TargetApi(19)
    public PinkFlutterView(@NonNull Context context, @NonNull FlutterImageView flutterImageView) {
        this(context, null, flutterImageView);
    }

    /**
     * Constructs a {@code PinkFlutterView} in an XML-inflation-compliant manner.
     *
     * <p>{@code PinkFlutterView} requires an {@code Activity} instead of a generic {@code Context} to be
     * compatible with {@link PlatformViewsController}.
     */
    // TODO(mattcarroll): expose renderMode in XML when build system supports R.attr
    public PinkFlutterView(@NonNull Context context, @Nullable AttributeSet attrs) {
    ...
    private void init() {
        Log.v(TAG, "Initializing PinkFlutterView");
        ...
        // PinkFlutterView needs to be focusable so that the InputMethodManager can interact with it.
        setFocusable(true);
     ...
     *
     * <ol>
     *   <li>{@code flutterEngineA} is attached to this {@code PinkFlutterView}: returns false
     *   <li>{@code flutterEngineA} renders its first frame to this {@code PinkFlutterView}: returns true
     *   <li>{@code flutterEngineA} is detached from this {@code PinkFlutterView}: returns false
     *   <li>{@code flutterEngineB} is attached to this {@code PinkFlutterView}: returns false
     *   <li>{@code flutterEngineB} renders its first frame to this {@code PinkFlutterView}: returns true
     * </ol>
     */
    ...
    /**
     * Adds the given {@code listener} to this {@code PinkFlutterView}, to be notified upon Flutter's
    ...
    /**
     * Invoked when this {@code PinkFlutterView} changes size, including upon initial measure.
     *
    ...
    protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
        ...
        Log.v(
                TAG,
                "Size changed. Sending Flutter new viewport metrics. PinkFlutterView was "
                        + oldWidth
                        + " x "
                        + oldHeight
                        + ", it is now "
     ...
     * text-input widgets, therefore {@code PinkFlutterView} must support an {@code InputConnection}.
     *
     * <p>The {@code InputConnection} returned from this method comes from a {@link TextInputPlugin},
     * which is owned by this {@code PinkFlutterView}. A {@link TextInputPlugin} exists to encapsulate the
    ...
    /**
     * Connects this {@code PinkFlutterView} to the given {@link FlutterEngine}.
     *
     * <p>This {@code PinkFlutterView} will begin rendering the UI painted by the given {@link
     * FlutterEngine}. This {@code PinkFlutterView} will also begin forwarding interaction events from
     * this {@code PinkFlutterView} to the given {@link FlutterEngine}, e.g., user touch events,
    ...
    public void attachToFlutterEngine(@NonNull FlutterEngine flutterEngine) {
        ...
        // with a PinkFlutterView that is only partially attached to a FlutterEngine.
...
//        sendViewportMetricsToFlutter();
    ...
    /**
     * Disconnects this {@code PinkFlutterView} from a previously attached {@link FlutterEngine}.
     *
     * <p>This {@code PinkFlutterView} will clear its UI and stop forwarding all events to the
    ...
    /**
     * Returns true if this {@code PinkFlutterView} is currently attached to a {@link FlutterEngine}.
     */
    @VisibleForTesting
    ...
    /**
     * Returns the {@link FlutterEngine} to which this {@code PinkFlutterView} is currently attached, or
     * null if this {@code PinkFlutterView} is not currently attached to a {@link FlutterEngine}.
     */
    @VisibleForTesting
    @Nullable
     ...
     * PinkFlutterView} attached to/detaches from a {@link FlutterEngine}.
     */
    @VisibleForTesting
        ...
        if (!isAttachedToFlutterEngine()) {
            Log.w(
                    ...
                    "Tried to send viewport metrics from Android to Flutter but this "
                            + "PinkFlutterView was not attached to a FlutterEngine.");
            return;
        }
    ...
    /**
     * Render modes for a {@link PinkFlutterView}.
     *
    ...
    public enum RenderMode {
         ...
         * mode has the best performance, but a {@code PinkFlutterView} in this mode cannot be positioned
         ...
         * This mode is not as performant as {@link RenderMode#surface}, but a {@code PinkFlutterView} in
         ...
         * performant as {@link RenderMode#surface}, but a {@code PinkFlutterView} in this mode can handle
    ...
    /**
     * Transparency mode for a {@code PinkFlutterView}.
     *
     ...
     * FlutterSurfaceView}, which is displayed when a {@code PinkFlutterView} uses {@link
     * RenderMode#surface}.
     *
     * <p>{@code TransparencyMode} does not impact {@link FlutterTextureView}, which is displayed when
     * a {@code PinkFlutterView} uses {@link RenderMode#texture}, because a {@link FlutterTextureView}
     * automatically comes with transparency.
     */
    @Deprecated
    public enum TransparencyMode {
        /**
         * Renders a {@code PinkFlutterView} without any transparency. This affects {@code PinkFlutterView}s in
        ...
        opaque,
        /**
         * Renders a {@code PinkFlutterView} with transparency. This affects {@code PinkFlutterView}s in {@link
     ...
     * {@code PinkFlutterView}.
     */
    @VisibleForTesting
    public interface FlutterEngineAttachmentListener {
        /**
         * The given {@code engine} has been attached to the associated {@code PinkFlutterView}.
         */
